...
> **Pflicht bei Finding-Fix:** Beim Schließen eines Findings (`Status` → `fixed`)
> sind die Spalten `test-file` und `verified-commit` Pflicht.
> Reproduktionstest beweist den Fix; Commit-Hash macht den Diff auffindbar.

| ID | Sev | Area | Title | File | Effort | Status | test-file | verified-commit |
|----|-----|------|-------|------|--------|--------|-----------|-----------------|
...
| F-112 | 🔴 | perf | Division durch Null in DurationEstimate.tsx | src/features/tournament-creation/components/DurationEstimate.tsx | 30m | fixed | src/features/tournament-creation/components/__tests__/DurationEstimate.test.tsx | b562562 |
| F-113 | 🔴 | data | startDate Validierung fehlt in Metadata | src/features/tournament-creation/Step3_Metadata.tsx | 20m | fixed | src/core/services/TournamentCreationService.test.ts | b562562 |
| F-114 | 🔴 | data | Team-Namen Validierung nicht vollständig | src/features/tournament-creation/Step4_Teams.tsx | 30m | fixed | src/core/services/TournamentCreationService.test.ts | b562562 |
| F-115 | 🔴 | ux | Mobile Team-Verwaltung zu komplex | src/features/tournament-creation/Step4_Teams.tsx | 2h | open | — | — |
| F-116 | 🔴 | data | Gruppe ohne Feld-Zuordnung möglich | src/features/tournament-creation/Step_GroupsAndFields.tsx | 45m | fixed | src/core/services/TournamentCreationService.test.ts | b562562 |
...
| F-211 | 🔴 | other | Schedule-Generation ohne Error-Handling kann App crashen | src/features/tournament-creation/TournamentPreview.tsx | 1h | fixed | src/features/tournament-creation/__tests__/TournamentPreview.scheduleError.test.tsx | b562562 |
...
| F-324 | 🟡 | architecture | loadData() flippt dataSource auf 'local' (Realtime-Flapping), wenn Cloud transient scheitert aber lokal ein Stand existiert | src/features/monitor-display/MonitorDisplayPage.tsx | 1h | open | — | — |
